...
# Bingo UHC II
execute if score #bhc bhc.scenario matches 02 if score #stepb_start bhc.data.temp matches ..0 if score #tracker_ennemies uhc.data.temp matches ..1000 if score #team uhc.data.temp.inv matches 3.. run function bhc:scenario/02/target/randomizer/countdown

# Rappel scénario
execute if score #stepb_start bhc.data.temp matches 5 if score #seconds uhc.data.temp matches 10 run function bhc:timer/countdown/reminder
