...
execute as @e[type=marker,tag=UHC,distance=0..,tag=!bhc.count,scores={uhc.player.lives=1..}] if score @s bhc.stepa.score.inv <= #temp bhc.data run scoreboard players add #count bhc.data.temp 1
