...
execute as @e[type=marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.death.tie_break.score.inv <= #temp bhc.data run scoreboard players add #count bhc.data.temp 1
