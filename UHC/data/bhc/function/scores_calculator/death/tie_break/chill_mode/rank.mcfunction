...
execute as @e[type=marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.chill_mode.tie_break.score <= #temp bhc.data run scoreboard players add #count bhc.data 1
tag @s remove bhc.count

## Classement
scoreboard players operation @s bhc.chill_mode.tie_break.rank = #team bhc.data
scoreboard players operation @s bhc.chill_mode.tie_break.rank -= #count bhc.data
