...
execute as @e[type=minecraft:marker,tag=UHC,distance=0..,scores={uhc.scenario.ironman=1..}] run function bhc:scores_calculator/death/ironman/scores
scoreboard players remove #tick bhc.data.temp 1

# Rangs
execute as @e[type=minecraft:marker,tag=UHC,distance=0..] run function bhc:scores_calculator/death/ironman/rank
