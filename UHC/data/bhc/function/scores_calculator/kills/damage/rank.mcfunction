...
execute as @e[type=marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.kills.damage_dealt.inv < #temp bhc.data run scoreboard players add #count bhc.data 1
tag @s remove bhc.count

## Classement
scoreboard players operation @s bhc.kills.damage_dealt.rank = #team bhc.data
scoreboard players operation @s bhc.kills.damage_dealt.rank -= #count bhc.data
