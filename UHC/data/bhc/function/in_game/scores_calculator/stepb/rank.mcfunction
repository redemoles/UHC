...
#> bhc:in_game/scores_calculator/stepb/rank
#
# @within			bhc:in_game/scenario/XX/bingo_stepb/team_reward/case
#
#
...
execute as @e[type=minecraft:marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.stepb.score.inv < #temp bhc.data run scoreboard players add #count bhc.data.temp 1
tag @s remove bhc.count

## Classement
scoreboard players operation @s bhc.stepb.rank.number = #team bhc.data.temp
scoreboard players operation @s bhc.stepb.rank.number -= #count bhc.data.temp

## Scores
scoreboard players operation @s bhc.stepb.rank.score.inv = #count bhc.data.temp
