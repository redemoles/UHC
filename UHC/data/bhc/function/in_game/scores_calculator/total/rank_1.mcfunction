...
#> bhc:in_game/scores_calculator/total/rank_1
#
# @within			bhc:in_game/scores_calculator/total/rank
#
#
...
execute as @e[type=minecraft:marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.total.score.inv < #temp bhc.data run scoreboard players add #count bhc.data.temp 1
