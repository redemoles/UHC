...
#> bhc:in_game/scores_calculator/death/ironman/rank
#
# @within			bhc:in_game/scores_calculator/death/ironman/bonus
#
#
...
execute as @e[type=minecraft:marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.ironman.timer.inv <= #temp bhc.data run scoreboard players add #count bhc.data.temp 1
