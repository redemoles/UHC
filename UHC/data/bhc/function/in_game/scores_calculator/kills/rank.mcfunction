...
#> bhc:in_game/scores_calculator/kills/rank
#
...
execute as @e[type=minecraft:marker,tag=UHC,distance=0..,tag=!bhc.count] if score @s bhc.kills.score.inv < #temp bhc.data run scoreboard players add #count bhc.data.temp 1
...
function bhc:in_game/scores_calculator/kills/damage/rank
