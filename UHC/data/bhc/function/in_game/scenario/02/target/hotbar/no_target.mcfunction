...
#> bhc:in_game/scenario/02/target/hotbar/no_target
#
# @within			bhc:in_game/scenario/02/target/hotbar/location
#
#
# @description		Ajout des joueurs de l'équipe dans la Hotbar de Target 
#

execute if score @s bhc.target.known matches 0 at @s if entity @p[tag=uhc.player,predicate=bhc:bhc_2/this_is_targeted,distance=..80] in uhc:lobby run function bhc:in_game/scenario/02/target/hotbar/target_reveal
