...
#> bhc:in_game/scenario/02/advancement/new_adv_1
#
# @within			bhc:in_game/scenario/02/advancement/new_adv
#
#
...
## Scores
$execute if score #stepa_enabled bhc.data matches $(step) run function bhc:in_game/scenario/02/bingo_stepa/team_reward/case with storage $(namespace) $(line)_$(column)
$execute if score #stepb_enabled bhc.data matches $(step) run function bhc:in_game/scenario/02/bingo_stepb/team_reward/case with storage $(namespace) $(line)_$(column)
...
$execute as @p[tag=bhc.new_adv] run function uhc:translation/bhc/02_viking_era/square_team with storage $(namespace) $(line)_$(column)
# Case - Pour tous
function bhc:in_game/advancement/player_can_receive_message/add
$function uhc:translation/bhc/02_viking_era/square_everyone with storage $(namespace) $(line)_$(column)
function bhc:in_game/advancement/player_can_receive_message/remove
# Ligne / Colonne
$execute unless score #team_first_completion bhc.data matches 1 if score #team_first_line_$(line) bhc.data matches 1 run function uhc:translation/bhc/02_viking_era/line with storage $(namespace) $(line)_$(column)
$execute unless score #team_first_completion bhc.data matches 1 if score #team_first_column_$(column) bhc.data matches 1 run function uhc:translation/bhc/02_viking_era/column with storage $(namespace) $(line)_$(column)
# Bingo complet
$execute if score #team_first_completion bhc.data matches 1 run function uhc:translation/bhc/02_viking_era/bingo_completed with storage $(namespace) $(line)_$(column)
