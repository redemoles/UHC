...
#> bhc:in_game/scenario/00/advancement/new_adv
#
...
## Scores
$execute if score #stepa_enabled bhc.data matches $(step) run function bhc:in_game/scenario/00/bingo_stepa/team_reward/case with storage $(namespace) $(line)_$(column)
$execute if score #stepb_enabled bhc.data matches $(step) run function bhc:in_game/scenario/00/bingo_stepb/team_reward/case with storage $(namespace) $(line)_$(column)
...
$execute as @p[tag=bhc.new_adv] run function uhc:translation/bhc/00_vanilla/square_team with storage $(namespace) $(line)_$(column)
# Case - Pour tous
$function uhc:translation/bhc/00_vanilla/square_everyone with storage $(namespace) $(line)_$(column)
# Ligne / Colonne
$execute unless score #team_first_completion bhc.data matches 1 if score #team_first_line_$(line) bhc.data matches 1 run function uhc:translation/bhc/00_vanilla/line with storage $(namespace) $(line)_$(column)
$execute unless score #team_first_completion bhc.data matches 1 if score #team_first_column_$(column) bhc.data matches 1 run function uhc:translation/bhc/00_vanilla/column with storage $(namespace) $(line)_$(column)
# Bingo complet
$execute if score #team_first_completion bhc.data matches 1 run function uhc:translation/bhc/00_vanilla/bingo_completed with storage $(namespace) $(line)_$(column)
