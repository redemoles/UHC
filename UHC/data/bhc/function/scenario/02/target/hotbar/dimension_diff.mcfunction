...
execute if score @s bhc.target.known matches 1 in uhc:lobby run data modify block 0 -61 0 front_text.messages[0] set value [{"text":" [","color":"#CFCFCF"},{"selector":"@e[type=marker,tag=UHC,distance=0..,predicate=bhc:bhc_2/this_is_targeted]"},{"text":" ?","color":"#FFFFFF","obfuscated":true},{"text":"]"}]
