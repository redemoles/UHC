...
execute as @s[type=minecraft:marker] if score @s bhc.target.team.live matches 0 unless score #tracker_ennemies uhc.data.temp matches 1.. run scoreboard players operation @e[type=minecraft:marker,tag=UHC,distance=0..,predicate=uhc:id_team] bhc.target.sort = @s uhc.id.team
...
scoreboard players set @s bhc.target.dead.temp 10
scoreboard players operation @s bhc.target.dead.temp *= #1200 uhc.data.numbers
