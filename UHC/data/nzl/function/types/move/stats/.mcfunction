...
execute as @s[tag=nzl.fee.bonus] unless entity @s[predicate=nzl:effects/champ_brumeux] if score @n[type=minecraft:marker,tag=UHC,predicate=uhc:id/team] nzl.attack.length.06 matches 0 run function nzl:types/move/attack_end/fee
