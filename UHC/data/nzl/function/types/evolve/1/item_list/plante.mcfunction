...
#> nzl:types/evolve/1/item_list/plante
#
# @within			nzl:types/evolve/1/disponible/plante
#
#
...
execute unless entity @s[scores={nzl.type.select=11,nzl.item1=0..,nzl.item2=0..,nzl.item3=0..,nzl.item4=0..,nzl.item5=0..},level=0..] run scoreboard players set @s nzl.type.select 99

# Echec
tellraw @s[scores={nzl.type.select=99}] [{"text":"Tu n'as pas tous les éléments requis pour le type ","color":"#FF3F3F","bold":false},{"text":"Plante","color":"#7FDF3F","bold":true},{"text":".","color":"#FF3F3F","bold":false}]
...
# Réussite
execute if score @s nzl.type.select matches 11 run function nzl:types/evolve/1/succes/plante with storage nzl plante_1_chat
