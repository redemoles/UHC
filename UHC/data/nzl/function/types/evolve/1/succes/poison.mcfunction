...
#> nzl:types/evolve/1/succes/poison
#
...
tag @s add nzl.type.12

# Équipe
scoreboard players operation #team uhc.id.team = @s uhc.id.team
tag @e[type=minecraft:marker,tag=UHC,predicate=uhc:id/team] add nzl.type.12
...
scoreboard players remove @n[type=minecraft:marker,tag=nzl.type,tag=12] nzl.type.available 1
scoreboard players remove #type_12 nzl.type.available 1

scoreboard players set @s nzl.type.evolve 0
scoreboard players set @s[scores={nzl.type.select=12}] nzl.type.player 12

## Préfix
function nzl:types/prefix/poison
