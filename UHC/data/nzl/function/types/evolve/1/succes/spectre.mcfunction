...
#> nzl:types/evolve/1/succes/spectre
#
...
tag @s add nzl.type.16

# Équipe
scoreboard players operation #team uhc.id.team = @s uhc.id.team
tag @e[type=minecraft:marker,tag=UHC,predicate=uhc:id/team] add nzl.type.16
...
scoreboard players remove @n[type=minecraft:marker,tag=nzl.type,tag=16] nzl.type.available 1
scoreboard players remove #type_16 nzl.type.available 1

scoreboard players set @s nzl.type.evolve 0
scoreboard players set @s[scores={nzl.type.select=16}] nzl.type.player 16

## Préfix
function nzl:types/prefix/spectre
