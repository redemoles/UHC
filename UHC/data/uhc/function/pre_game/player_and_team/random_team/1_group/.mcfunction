
#> uhc:pre_game/player_and_team/random_team/1_group/
#
# @within			uhc:pre_game/tick
#
#
# @description		Génération d'équipes aléatoires
#


#tellraw @a[tag=uhc.host] [{"text":"","bold":false},{"text":"[Test]","bold":true},{"text":" Démarrage du groupe de function"}]
execute as @a unless score @s uhc.id.team matches 99 run team join 091 @s
scoreboard players set @a[team=091] uhc.id.team 91

tag @a[tag=uhc.player] add uhc.random_team.in_a_team

scoreboard players set @a[tag=uhc.player] uhc.id.random_team 0
execute store result score #team uhc.id.team run random value 0..15
#scoreboard players set #team uhc.id.team 0
scoreboard players set #team uhc.id.random_team 1
scoreboard players set #count uhc.id.random_team 1
execute as @a[tag=uhc.player] unless score @s uhc.id.random_team.ban matches 1.. run scoreboard players set @s uhc.id.random_team.ban 0

function uhc:pre_game/player_and_team/random_team/1_group/selector
execute as @r[tag=uhc.player] run function uhc:pre_game/player_and_team/random_team/1_group/color
