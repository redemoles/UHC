
#> uhc:pre_game/menu/selected/main/player/team/chosen/01-16/06
#
# @within			uhc:pre_game/menu/selected/main/player/gm
#
#
# @description		Formation des équipes
#

execute store result score #temp uhc.data.setup if entity @a[scores={uhc.id.team=06}]
execute if score #temp uhc.data.setup >= #team_size uhc.data.setup run return run function uhc:translation/menu/load/team/page/team_join/cancel_team_full

tag @s remove uhc.spec
tag @s add uhc.player
scoreboard players set @s uhc.id.team 06
execute if score #anonyme_team uhc.data.setup matches 0 run team join 006 @s
execute if score #anonyme_team uhc.data.setup matches 1 run team join 091 @s

function uhc:translation/menu/load/team/page/team_join/01-16/06
