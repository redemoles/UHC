...
execute if score #game_progress uhc.game_progress matches 1.. as @s[tag=uhc.revive.temp,gamemode=!creative] in uhc:lobby run function uhc:in_game/player/spec/revive/new_player/menu/selection/root
execute if score #game_progress uhc.game_progress matches 1.. run return fail

execute as @s[scores={uhc.player.online=1},gamemode=!creative] unless entity @s[tag=!mgs.tc.spec,tag=mgs.tc.player] unless score @s uhc.menu.settings.inventory matches 2..6 run function uhc:pre_game/menu/detector/menu
execute as @s[tag=!mgs.tc.spec,tag=mgs.tc.player,scores={uhc.player.online=1},gamemode=!creative] unless score @s uhc.menu.settings.inventory matches 2..6 run return run function lobby:mini_games/inventory
...
execute if score @s uhc.player.online matches 1 if score @s uhc.menu.settings.inventory matches 6 run function uhc:pre_game/menu/load/settings/inventory/start_and_rewards/item_life_loss/new_change
