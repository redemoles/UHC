...
execute as @s[scores={uhc.player.online=1},gamemode=!creative] unless entity @s[tag=!mgs.tc.spec,tag=mgs.tc.player] unless score @s uhc.menu.settings.inventory matches 2..10 if score #minutes uhc.data.temp matches -1 if score #seconds uhc.data.temp matches -1 run function uhc:pre_game/menu/detector/menu
