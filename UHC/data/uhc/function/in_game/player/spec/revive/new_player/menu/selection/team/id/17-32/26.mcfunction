...
execute store result score #temp uhc.data.setup run scoreboard players get @n[type=minecraft:marker,tag=UHC,distance=0..,scores={uhc.id.team=26}] uhc.team.size
...
execute if score #anonyme_team uhc.data.setup matches 0 run team join 026 @s
execute if score #anonyme_team uhc.data.setup matches 1 run team join 091 @s

scoreboard players enable @s uhc.menu.revive
tellraw @s[scores={uhc.player.lang=061801}] [{"text":"\nClique sur ce message pour rejoindre l'","color":"#FFFFFF","click_event":{"action":"run_command","command":"trigger uhc.menu.revive set 26"}},{"text":"♦ Équipe Rouge f.","color":"dark_red"}]
tellraw @s[scores={uhc.player.lang=051407}] [{"text":"\nClick on this message to join the ","color":"#FFFFFF","click_event":{"action":"run_command","command":"trigger uhc.menu.revive set 26"}},{"text":"♦ D. Red Team","color":"dark_red"}]
