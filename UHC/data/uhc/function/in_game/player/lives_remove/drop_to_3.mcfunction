...
#> uhc:in_game/player/lives_remove/drop_to_3
#
# @within			uhc:in_game/timer/tick
# 
#
# @description		Diminution à 3 vies maximum et compensations
#
...
execute as @a[scores={uhc.player.lives=4..},predicate=uhc:id/team] run function uhc:in_game/player/misc/team_join
...
execute if score #bhc uhc.gamemode matches 1 as @a[scores={uhc.player.lives=4..},predicate=uhc:id/team] run function uhc:translation/bhc/player_lives_remove_drop_to_3
...
execute if score #anonyme_team uhc.data.setup matches 1 as @a[scores={uhc.player.lives=4..},predicate=uhc:id/team] run team join 091 @s
execute as @a[scores={uhc.player.lives=4..},predicate=uhc:id/team] run function #plugin:in_game/player/team_join

## Récompense
# Tous les modes de jeu
give @a[scores={uhc.player.lives=4..},predicate=uhc:id/team] minecraft:golden_apple 2

# Bingo UHC
execute if score #bhc uhc.gamemode matches 1 store result score #count bhc.data.temp if entity @a[scores={uhc.player.lives=4..},predicate=uhc:id/team]
execute if score #bhc uhc.gamemode matches 1 run scoreboard players operation @s bhc.team.livescount += #count bhc.data.temp

## Retrait vie
scoreboard players set @a[scores={uhc.player.lives=4..},predicate=uhc:id/team] uhc.player.lives 3
