
#> uhc:in_game/inventory/ban_item/potion/regeneration
#
# @within			uhc:in_game/inventory/ban_item/root
#
#
# @description		Suppression des potions de régénération
#

scoreboard players set #temp uhc.data.temp 0
execute if items entity @s container.* *[potion_contents={potion:"minecraft:regeneration"}] run scoreboard players set #temp uhc.data.temp 1
execute if items entity @s container.* *[potion_contents={potion:"minecraft:long_regeneration"}] run scoreboard players set #temp uhc.data.temp 1
execute if items entity @s container.* *[potion_contents={potion:"minecraft:strong_regeneration"}] run scoreboard players set #temp uhc.data.temp 1
execute if score #temp uhc.data.temp matches 0 run return fail
clear @s *[potion_contents={potion:"minecraft:regeneration"}] 1
clear @s *[potion_contents={potion:"minecraft:long_regeneration"}] 1
clear @s *[potion_contents={potion:"minecraft:strong_regeneration"}] 1
function uhc:in_game/inventory/ban_item/potion/regeneration
