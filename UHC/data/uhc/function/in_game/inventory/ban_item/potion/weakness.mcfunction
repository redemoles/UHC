...
#> uhc:in_game/inventory/ban_item/potion/weakness
#
# @within			uhc:in_game/inventory/ban_item/root
#
#
# @description		Suppression des potions de faiblesse
#

scoreboard players set #temp uhc.data.temp 0
execute if items entity @s container.* *[potion_contents={potion:"minecraft:weakness"}] run scoreboard players set #temp uhc.data.temp 1
...
clear @s *[potion_contents={potion:"minecraft:weakness"}] 1
clear @s *[potion_contents={potion:"minecraft:long_weakness"}] 1

function uhc:in_game/inventory/ban_item/potion/weakness
